...
if(isset($_POST["btnSubmit"]) && $_POST["btnSubmit"]=='Add Help' && !$error) {
    ...
    header('location:help_admin.php?msg=a&ddlCategory='.$ddlCategory);
    exit();
}
...
if(isset($_POST["btnSubmit"]) && $_POST["btnSubmit"]=='Edit Help' && !$error) {
    ...
    header('location:help_admin.php?msg=e&ddlCategory='.$ddlCategory);
    exit();
}
...
$langSql     = "SELECT lang_id,lang_name,folder_name FROM " . TABLEPREFIX . "lang WHERE lang_status = 'y'";
$langRs      = mysqli_query($conn, $langSql);

//fetch category from Help category table
$category   = mysqli_query($conn, "SELECT * FROM " . TABLEPREFIX . "helpcategory_lang L
                             JOIN " . TABLEPREFIX . "helpcategory H
                               ON L.help_cat_id = H.nHcId
                              AND L.lang_id = '".$_SESSION["lang_id"]."'
                              AND H.vHtype='admin' AND H.vActive='1' ORDER BY H.nHcposition ASC") or die(mysqli_error($conn));
                                        ...
                                        <?php if(isset($message) && $message!='') {
                                         ...
                                         <?php  }?>
                                            ...
                                            <tr bgcolor="#FFFFFF">
                                                ...
                                                <td colspan="2"><select name="ddlCategory" class="textbox2">
                                            <?php
                                                                                        
                                            if(mysqli_num_rows($category)>0) {
                                                
                                                while($arr=mysqli_fetch_array($category)) {
                                                    if($arr['help_cat_id'] == $ddlCategory){
                                                                $selected = "selected";
                                                            }else{
                                                                $selected = "";
                                                            }
                                                    ?>
                                                    <option value="<?php echo $arr['nHcId'];?>" <?php echo $selected;?>><?php echo $arr['vHctitle'];?></option>
                                                    <?php
                                                    $k++;
                                                }
                                            }
