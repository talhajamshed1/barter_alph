...
if(isset($_GET['mode']) && $_GET['mode']=='delete')
{
	...
	header('location:help.php?msg=d&ddlCategory='.$ddlCategory);
	exit();
}//end if
	...
	if(isset($_GET['move']) && $_GET['move']=='down')
	{
        ...
        */
        if(isset($_GET['move'])) {
            ...
            $sqlO = "SELECT h.*,hp.*,hc.*,hct.vHctitle,h.vActive as ha,h.nHposition as nPos
                ...
                <tr>
                  ...
                  <td width="78%" valign="top">
                    ...
                    <table width="100%"  border="0" cellspacing="0" cellpadding="10">
                      <tr>
                        <td width="74%" class="heading_admn boldtextblack" align="left">Client Help </td>
                        <td width="26%" align="right" class="heading_admn"><a href="add_help.php<?php if($ddlCategory!=''){echo '?ddlCategory='.$ddlCategory;}?>" class="AddLinks">Add New Help</a></td>
                      </tr>
                    </table>
                    <table width="100%"  border="0" cellspacing="0" cellpadding="10">
                      <tr>
	...
	$sql = "SELECT h.*,hp.*,hct.vHctitle,hc.*,h.vActive as ha,h.nHposition as nPos
		    ...
		    ON h.nHcId=hc.nHcId
                  ...
                  JOIN ".TABLEPREFIX."help_lang hp
                   ...
                   AND hp.lang_id = '".$_SESSION["lang_id"]."'
                   AND hct.lang_id = '".$_SESSION["lang_id"]."'
                 where hc.vHtype='client' ORDER BY hc.nHcposition ASC,h.nHposition ASC";
}//end else
    ...
    <td align="right">Filter By Category : <select name="ddlCategory" class="textbox2" onChange="document.frmHelp.submit();">
			...
			$language=mysqli_query($conn, "SELECT * FROM ".TABLEPREFIX."helpcategory H
                                                        JOIN ".TABLEPREFIX."helpcategory_lang L
                                                         ...
                                                         AND H.vActive='1' and H.vHtype='client' order by H.nHcposition desc") or die(mysqli_error($conn));
...
<?php  }//end if?>			
	...
	<a href='help.php?Action=ordering&move=up&id=<?php echo $arr["nHId"];?>&pos=<?php echo $arr["nPos"];?><?php echo $order_category;?>'>

<img src="../images/up.gif" alt="Up" border=0></a>
<?php } ?>
<?php if($cnt!=$totalrows){ ?>
<a href="help.php?Action=ordering&move=down&id=<?php echo $arr["nHId"];?>&pos=<?php echo $arr["nPos"];?><?php echo $order_category;?>">
<img src="../images/down.gif" alt="Down" border=0></a><?php } ?></td>
                                <td align="center"><?php if($arr['ha']=='1'){echo 'Yes';}else{echo 'No';}?></td>
                                <td align="center"><?php echo "<a href='add_help.php?nHId=".$arr["nHId"]."&mode=edit".$order_category."'>Edit</a>";?>&nbsp;|&nbsp;<?php echo "<a href='help.php?nHId=".$arr["nHId"]."&mode=delete".$order_category."' onClick='javascript:return confirm(\"Are you sure you want to delete this?\");'>Delete</a>";?></td>
                              </tr>
					<?php 
								$cnt++;
							}//end while
