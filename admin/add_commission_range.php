...
if($_GET['mode']=='edit') {
    $sql=mysqli_query($conn, "select * from ".TABLEPREFIX."escrowrangefee where nLId='".$_GET['nLId']."'") or die(mysqli_error($conn));
    ...
    $defaultValue=fetchSingleValue(select_rows(TABLEPREFIX.'escrowrangefee','nTo',"WHERE vActive='1' AND nLId='".$oldId."'"),'nTo');
...
if(isset($_POST["btnSubmit"]) && $_POST["btnSubmit"]=='Add Range') {
    ...
    $sql = mysqli_query($conn, "SELECT MAX(nLPosition) as max from ".TABLEPREFIX."escrowrangefee") or die(mysqli_error($conn));
    ...
    if($_POST["rangeType"]=="rangeBetween") {
        ...
        if($message==''){
        mysqli_query($conn, "insert into ".TABLEPREFIX."escrowrangefee (nPrice,nFrom,nTo,nLPosition,vActive) values
						('".addslashes($txtAmount)."','".addslashes($txtFrom)."','".addslashes($txtTo)."',
						'".$maxorder."','".addslashes($radActive)."')") or die(mysqli_error($conn));

        header('location:manage_commission_range.php?msg=a');
        exit();
        }
    }else if($_POST["rangeType"]=="above"){
        $maxvalue = fetchSingleValue(select_rows(TABLEPREFIX.'escrowrangefee','max(nTo) as max_value',"WHERE vActive='1'"),'max_value');
        $message = '';
        ...
        if($message==''){
         mysqli_query($conn, "insert into ".TABLEPREFIX."escrowrangefee (nPrice,above,nLPosition,vActive) values
...
if(isset($_POST["btnSubmit"]) && $_POST["btnSubmit"]=='Edit Range') {
    ...
    if($_POST["rangeType"]=="rangeBetween") {
        ...
        if(checkEscrowRange($txtFrom,$nLId)==1) {
            $message.='From range already exists<br>';
        }//end if

        //checking range already exists
        if(checkEscrowRange($txtTo,$nLId)==1) {
            $message.='To range already exists<br>';
        }//end if
        else {//checking new range
            if(checkNewEscrowRange($txtFrom,$txtTo,$nLId)==1) {
//                $message.='Given range already exists<br>';
                $message.='Certain values in the given range already exists<br>. Please check the existing escrow fee range and add a valid range.<br>';
            }//end if
        }//end else
        if($txtTo<$txtFrom) {
        ...
        if($message=='') {
            mysqli_query($conn, "update ".TABLEPREFIX."escrowrangefee set nFrom='".addslashes($txtFrom)."',
                                                             nPrice='".addslashes($txtAmount)."',nTo='".addslashes($txtTo)."',above='0' WHERE nLId='".$nLId."'") or die(mysqli_error($conn));
            header('location:manage_commission_range.php?msg=e');
            exit();
        }
    }else if($_POST["rangeType"]=="above"){
        $maxvalue = fetchSingleValue(select_rows(TABLEPREFIX.'escrowrangefee','max(nTo) as max_value',"WHERE vActive='1'"),'max_value');
        $checkAbove = fetchSingleValue(select_rows(TABLEPREFIX.'escrowrangefee','above',"WHERE vActive='1' AND nLId='".$nLId."'"),'above');
        $above = $_POST["aboveRange"];
        if($above=="") {
            $message.='Please enter a value to above range';
        }//end if
        ...
        if($message==''){
            ...
            mysqli_query($conn, "update ".TABLEPREFIX."escrowrangefee set nFrom='0',nTo='0',
    ...
    <tr>
        ...
        <td width="78%" valign="top"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
            ...
            <table width="100%"  border="0" cellspacing="0" cellpadding="10">
                <tr>
                    <td width="84%" class="heading_admn boldtextblack" align="left"><?php echo $btnVal;?> Range</td>
                ...
                <table width="100%"  border="0" cellspacing="0" cellpadding="10">
                    <tr>
                        <td align="left" valign="top"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    <td><table width="100%"  border="1" cellspacing="0" cellpadding="4" class="maintext2">
                                            ...
                                            <form name="frmListing" method ="POST" action = "" onSubmit="return category_chk()" enctype="multipart/form-data">
                                                ...
                                                <tr align="right" bgcolor="#FFFFFF">
                                                    <td colspan="3"><a style="margin-right: 8px;" href="manage_commission_range.php"><b>Back</b></a></td>
                                                </tr>
                                                <tr bgcolor="#FFFFFF">
                                                <input type="hidden" name="nLId" value="<?php echo $_GET['nLId'];?>">
                                                <td style="padding: 7px 7px" colspan="2" align="left" valign="top" class="warning"> * indicates mandatory fields</td>
                                                <td width="45%" align="left" valign="top" class="warning"><span id="txtRange"></span></td>
                                                </tr>

                                                 <tr bgcolor="#FFFFFF">
                                                    <td width="18%"  style="padding: 7px 7px" valign="top">Selcet Range</td>
                                                    <td colspan="2"  style="padding: 7px 7px">
                                                        ...
                                                        <div id="range" <?php if($above || $above!=0){?>style="display: none;"<?php }?>>
                                                            <table border="0" cellpadding="0"  width="100%">
                                                                <tr bgcolor="#FFFFFF">
                                                                        <td width="18%" align="left" valign="middle">Range </td>
                                                                        <td colspan="2" align="left" valign="middle">From <?php echo CURRENCY_CODE;?>
                                                                            <input type="text" class="textbox2" name="txtFrom" size="5" maxlength="8" <?php if($mode=="add"){?>readonly <?php }?>value="<?php echo round($txtFrom,2);?>" onKeyUp="showEscrowRange('f',this.value,'<?php echo $mode;?>','<?php echo $_GET['nLId'];?>');" onMouseDown="showEscrowRange('f',this.value,'<?php echo $mode;?>','<?php echo $_GET['nLId'];?>');"/>								To <?php echo CURRENCY_CODE;?>
                                                                            <input type="text" class="textbox2" name="txtTo" size="5" maxlength="8" value="<?php if($mode=="edit")echo round($txtTo,2);?>" onKeyUp="showEscrowRange('t',this.value,'<?php echo $mode;?>','<?php echo $_GET['nLId'];?>');" onMouseDown="showEscrowRange('t',this.value,'<?php echo $mode;?>','<?php echo $_GET['nLId'];?>');"/>
                                                                        </td>
                                                                </tr>
                                                            </table>
                                                        </div>
                                                        <div id="above" <?php if($above=="" || $above==0 ){?>style="display: none;"<?php }?>>
                                                            <table>
                                                                <tr bgcolor="#FFFFFF">
                                                                    <td align="left" valign="middle">Above &nbsp; </td>
                                                ...
                                                <tr bgcolor="#FFFFFF">
                                                    <td align="left"  style="padding: 7px 7px" valign="top">Listing Price (%) <span class="warning">*</span></td>
                                                    <td  style="padding: 7px 7px" colspan="2" align="left" valign="top"><input type="text" class="textbox2" name="txtAmount" size="5" value="<?php echo $txtAmount;?>"/></td>
                                                </tr>
                                                ...
                                                <tr bgcolor="#FFFFFF">
                                                    <td align="left">&nbsp;</td>
                                                    <td  style="padding: 7px 7px" colspan="2">
                                                        <?php 
