...
switch($vmode)
{
	case "add":
		$appendCndt='';
                $numRows = select_rows(TABLEPREFIX.'listingfee',"nTo","WHERE vActive='1'");
                if($numRows){
                    $defaultValue=fetchSingleValue(select_rows(TABLEPREFIX.'listingfee','nTo',"WHERE vActive='1' ORDER BY nTo DESC limit 0,1"),'nTo');
                }
                if($defaultValue==0){
                    $defaultValue=fetchSingleValue(select_rows(TABLEPREFIX.'listingfee','max(nTo) as maxTo',"WHERE vActive='1'"),'maxTo');
                }
	...
	case "edit":
		...
		$fetchField='nTo';
                /*$numRows = select_rows(TABLEPREFIX.'listingfee',$fetchField,"WHERE vActive='1' ".$appendCndt."");
                if($numRows){
		...
		//fetch old values
		$txtFromOld=fetchSingleValue(select_rows(TABLEPREFIX.'listingfee','nFrom',"WHERE nLId='".$nrId."'"),'nFrom');
		$txtToOld=fetchSingleValue(select_rows(TABLEPREFIX.'listingfee','nTo',"WHERE nLId='".$nrId."'"),'nTo');
	break;
}//end switch
...
//				$message='From range should be '.$defaultValue.' - '.$newValu;
                            $message='From range should be '.$defaultValue;
...
//					$message.='<br>From range should be '.$defaultValue.' - '.$newValu;
                                        $message.='<br>From range should be '.$defaultValue;
				}//end if
			}//end if
		}//end else
	break;
