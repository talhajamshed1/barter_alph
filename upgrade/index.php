...
$schemafile = "schema_upgrade2.0.sql";
$datafile = "data_upgrade2.0.sql";
$configfile = "../includes/config.php";

$logoimage="logo.gif";
$imagefolder =  "../images";
$stylesfolder = "../styles";
$sitestyle="default.css";
$logourl="logo.gif";
$productname=SITE_NAME;

$configcontents = @fread(@fopen($configfile, 'r'), @filesize($configfile));
$pos = strpos($configcontents, "UPGRADED");
if ($pos===false) 
{
	;
}//end if
...
$directories = array("../help/","../includes/config.php","../banners/","../pem/");
umask(0);
...
{	
	if($OS != "WIN")
	{
		...
		{
			$error = true;
			$message .= " * Change the permission (chmod 777 for linux server) of 'help' folder<br>";
		}//end if
		if (!is_writable($bannersdir) || !is_readable($bannersdir) || !@file_exists("$bannersdir".".")) 
		{
			$error = true;
			$message .= " * Change the permission (chmod 777 for linux server) of 'banners' folder<br>";
		}//end if
 ...
 //php5.4 fix
	...
	else 
	{
		...
		//retreving directory name
   		$documentroot=str_replace("/upgrade","",dirname($_SERVER['PHP_SELF']));
		
		//store config file details
		$configfile_content=file_get_contents($configfile);

		$final_configcontent = "<?php\n";
		$final_configcontent .= "ob_start(); \n\n";
        $final_configcontent .= "define('INSTALLED', true); \n\n";
		$final_configcontent .= "define('UPGRADED', true); \n\n";
		$final_configcontent .= "define('SITE_URL', '$txtSiteURL'); \n\n";
		$final_configcontent .= '$HOST = "' . $var_host . "\";" . "\n";
        $final_configcontent .= '$DATABASENAME = "' . $var_database . "\";" . "\n";
        $final_configcontent .= '$USER = "' . $var_user . "\";" . "\n";
        $final_configcontent .= '$PASSWORD = "' . $var_password . "\";" . "\n\n";
	    $final_configcontent .= '$txtDBPrefix = "' . $txtDBPrefix . "\";" . "\n";
		$final_configcontent .= '$imagefolder = SITE_URL . "/images";'."\n";
		$final_configcontent .= '$stylesfolder = SITE_URL . "/themes";'."\n";
		$final_configcontent .= '$rootserver = "' . $var_server . "\";" . "\n\n\n";
		$final_configcontent .='$rootserver = SITE_URL;'."\n\n\n";
		$final_configcontent .='//db connection'."\n";
		$final_configcontent .='$conn = mysqli_connect($HOST, $USER, $PASSWORD);'."\n";
		$final_configcontent .='mysqli_select_db($conn, $DATABASENAME);'."\n\n";
		$final_configcontent .="define('TABLEPREFIX', '$txtDBPrefix');"."\n\n";
		$final_configcontent .='$sql = "Select nLookUpCode,vLookUpDesc from ".$txtDBPrefix."lookup WHERE nLookUpCode IN(\'4\',';
		$final_configcontent .='\'sitetitle\',\'sitename\',\'sitelogo\',\'sitestyle\',\'surl\',\'SessionTimeout\',\'currency\',\'currencycode\')";'."\n";
		$final_configcontent .='$rs = mysqli_query($conn, $sql) or die(mysqli_error($conn));'."\n";
		$final_configcontent .='if (mysqli_num_rows($rs) > 0)'."\n";
		$final_configcontent .='{'."\n";
		$final_configcontent .="\t".'while($row = mysqli_fetch_array($rs))'."\n";
		$final_configcontent .="\t\t".'{'."\n";
		$final_configcontent .="\t\t\t".'switch($row["nLookUpCode"])'."\n";
		$final_configcontent .="\t\t\t\t".'{'."\n";
		$final_configcontent .="\t\t\t\t\t".'case 4:'."\n";
		$final_configcontent .="\t\t\t\t\t\t".'define("SITE_EMAIL", htmlentities($row["vLookUpDesc"]));'."\n";
		$final_configcontent .="\t\t\t\t\t\t".'define("ADMIN_EMAIL", htmlentities($row["vLookUpDesc"]));'."\n";
		$final_configcontent .="\t\t\t\t\t".'break;'."\n";
		$final_configcontent .="\t\t\t\t\t".'case "sitetitle":'."\n";
		$final_configcontent .="\t\t\t\t\t\t".'define("SITE_TITLE", htmlentities($row["vLookUpDesc"]));'."\n";
		$final_configcontent .="\t\t\t\t\t".'break;'."\n";
		$final_configcontent .="\t\t\t\t\t".'case "sitename":'."\n";
		$final_configcontent .="\t\t\t\t\t\t".'define("SITE_NAME", htmlentities($row["vLookUpDesc"]));'."\n";
		$final_configcontent .="\t\t\t\t\t".'break;'."\n";
		$final_configcontent .="\t\t\t\t\t".'case "sitelogo":'."\n";
		$final_configcontent .="\t\t\t\t\t\t".'$logourl  = $row["vLookUpDesc"];'."\n";
		$final_configcontent .="\t\t\t\t\t".'break;'."\n";
		$final_configcontent .="\t\t\t\t\t".'case "sitestyle": '."\n";
		$final_configcontent .="\t\t\t\t\t\t".'$sitestyle = $row["vLookUpDesc"];'."\n";
		$final_configcontent .="\t\t\t\t\t".'break;'."\n";
		$final_configcontent .="\t\t\t\t\t".'case "surl": '."\n";
		$final_configcontent .="\t\t\t\t\t\t".'define("SECURE_SITE_URL", htmlentities($row["vLookUpDesc"]));'."\n";
		$final_configcontent .="\t\t\t\t\t".'break;'."\n";
		$final_configcontent .="\t\t\t\t\t".'case "SessionTimeout": '."\n";
		$final_configcontent .="\t\t\t\t\t\t".'define("SESSION_TIMEOUT", htmlentities($row["vLookUpDesc"]));'."\n";
		$final_configcontent .="\t\t\t\t\t".'break;'."\n";
		$final_configcontent .="\t\t\t\t\t".'case "currency": '."\n";
		$final_configcontent .="\t\t\t\t\t\t".'define("CURRENCY_CODE", $row["vLookUpDesc"]);'."\n";
		$final_configcontent .="\t\t\t\t\t".'break;'."\n";
		$final_configcontent .="\t\t\t\t\t".'case "currencycode": '."\n";
		$final_configcontent .="\t\t\t\t\t\t".'define("PAYMENT_CURRENCY_CODE", $row["vLookUpDesc"]);'."\n";
		$final_configcontent .="\t\t\t\t\t".'break;'."\n";
		$final_configcontent .="\t\t\t\t".'}'."\n";
		$final_configcontent .="\t\t".'}'."\n";
		$final_configcontent .='}'."\n";
		$final_configcontent .='$secureserver=SECURE_SITE_URL;'."\n";
        $final_configcontent .= "\n?>";

		//replace installed with upgrad with new values
		$final_configcontent=str_replace($configfile_content,$final_configcontent,$configfile_content);

		//final result write into config.inc.php
		if(@file_exists($configfile))
		{
			//rename old file
			@rename('old_'.$configfile);
		}//end if
		
		//changes write into config.inc.php
		@fwrite(@fopen($configfile,'w+'),$final_configcontent);
		...
		//update sale,swap n saleextra table
		$sale=mysqli_query($connection, "select vUrl from ".$txtDBPrefix."sale") or die(mysqli_error($connection));
		if(mysqli_num_rows($sale)>0)
		{
			while($arr=mysqli_fetch_array($sale))
			{
				mysqli_query($connection, "update ".$txtDBPrefix."sale set vSmlImg='".$arr['vUrl']."' where vUrl='".$arr['vUrl']."'")
										 or die(mysqli_error($connection));
			}//end while
		}//end if

		$swap=mysqli_query($connection, "select vUrl from ".$txtDBPrefix."swap") or die(mysqli_error($connection));
		if(mysqli_num_rows($swap)>0)
		{
			while($arr=mysqli_fetch_array($swap))
			{
				mysqli_query($connection, "update ".$txtDBPrefix."swap set vSmlImg='".$arr['vUrl']."' where vUrl='".$arr['vUrl']."'")
										 or die(mysqli_error($connection));
			}//end while
		}//end if

		$saleextra=mysqli_query($connection, "select vUrl from ".$txtDBPrefix."saleextra") or die(mysqli_error($connection));
		if(mysqli_num_rows($saleextra)>0)
		{
			while($arr=mysqli_fetch_array($saleextra))
			{
				mysqli_query($connection, "update ".$txtDBPrefix."saleextra set vSmlImg='".$arr['vUrl']."' where vUrl='".$arr['vUrl']."'")
										 or die(mysqli_error($connection));
			}//end while
		}//end if

		//update category position
		$categorypos=mysqli_query($connection, "select nCategoryId from ".$txtDBPrefix."category") or die(mysqli_error($connection));
		if(mysqli_num_rows($categorypos)>0)
		{
			$count='1';
			while($arr=mysqli_fetch_array($categorypos))
			{
				mysqli_query($connection, "update ".$txtDBPrefix."category set nPosition='".$count."' where nCategoryId='".$arr['nCategoryId']."'")
										 or die(mysqli_error($connection));
				$count++;
			}//end while
		}//end if

		 //update listing fee and range to new table and remove from old
		 $sql=mysqli_query($connection, "select vLookUpDesc from ".$txtDBPrefix."lookup where nLookUpCode='8'") or die(mysqli_error($connection));
		if(mysqli_num_rows($sql)>0)
		{
			$existListingRange=mysqli_result($sql,0,'vLookUpDesc');
		}//end if
		
		 $sql=mysqli_query($connection, "select vLookUpDesc from ".$txtDBPrefix."lookup where nLookUpCode='8'") or die(mysqli_error($connection));
		if(mysqli_num_rows($sql)>0)
		{
			$existListingComm=mysqli_result($sql,0,'vLookUpDesc');
		}//end if

		//insert into rang table
		mysqli_query($connection, "insert into ".$txtDBPrefix."listingfee (nPrice,nFrom,nTo,nLPosition,vActive) values 
					('".addslashes($existListingComm)."','".addslashes($existListingComm)."','5000',
					'1','1')") or die(mysqli_error($connection));
          ...
          <tr>
            <td width="67%" align="left" valign="middle" class="linkbar"><table width="60%"  border="0" cellspacing="0" cellpadding="2">
              <tr align="center" class="link">
                <td width="8%"><a class="listing" title="OnlineInstallationManual" href="#" onClick="window.open('<?php echo htmlentities($txtSiteURL);?>/docs/eswap.pdf','OnlineInstallationManual','top=100,left=100,width=820,height=550,scrollbars=yes,toolbar=no,status=yrd');"><strong>Upgradation Manual</strong></a></td>
                        ...
                        <td align="left" valign="top"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            <td bgcolor="#EEEEEE"><table width="100%"  border="0" cellspacing="1" cellpadding="4" class="maintext2">
							  ...
							  <tr bgcolor="#FFFFFF">
                                ...
                                <td width="73%" align="left"><?php echo $ivo.PHP_VERSION.$sc . " ";
									...
									{
										echo $ivf."(4.2.0 or higher required)".$sc; $fatal = true;
									}//end else
									?>
								</td>
							  </tr>
							...
							?>
								...
								&nbsp;&nbsp;&nbsp;&nbsp;2. Change the permission (chmod 777 for linux server) of 'help' folder. <br>
								&nbsp;&nbsp;&nbsp;&nbsp;3. Change the permission (chmod 777 for linux server) of 'banners' folder. <br>
								&nbsp;&nbsp;&nbsp;&nbsp;4. Change the permission (chmod 777 for linux server) of 'pem' folder. <br>
								&nbsp;&nbsp;&nbsp;&nbsp;5. After the upgradation delete the 'upgrade' folder and its contents.
								<?php }//end if
								  else
								  {
								  		echo 'Please note the following points before you continue:<br>';
								  		echo "&nbsp;&nbsp;&nbsp;&nbsp;1. After the upgradation delete the 'upgrade' folder and its contents.";
								  }//end else
						   ?>				</td>
                              </tr>
							...
							?>
							  ...
							  <td colspan="2"><b>To ensure complete security, now <br> 
                                  a)You should remove the 'upgrade' directory.<br>
