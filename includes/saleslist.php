...
// | webdesign etc.                                                       |
// +----------------------------------------------------------------------+
function func_sale_list($uid = 0, $fid = 0) {
    global $numRecordsPage,$numPageLinks,$conn;
 $recStart = $_GET['begin'];
  if($_GET['page']!='saleslist')
    $_GET['begin'] = $_SESSION['pagenav']['saleslist'];
  else
     $_SESSION['pagenav']['saleslist'] = $_GET['begin'];
     
                     $sql = "SELECT s.nSaleId,s.vTitle,date_format(dPostDate,'%m/%d/%Y') as
                'dPostDate',s.vFeatured, L.vCategoryDesc,s.nCategoryId FROM " . TABLEPREFIX . "sale s
        ...
        $totalrows = mysqli_num_rows($result);
          ...
          //$navigate = pageBrowser($totalRecords, $numPageLinks, $numRecordsPage, "&page=saleslist", $_GET[numBegin], $_GET[start], $_GET[begin], $_GET[num]);
...
if($pagenumber != 1) {  $cnt = PAGINATION_LIMIT+1; }
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            ?>
                                ...
                                <td colspan="2" align="left"><?php echo "<a href='" . $detailfile . "?saleid=" . $row["nSaleId"] . "&source=sa' class='$style'>" . htmlentities($row["vCategoryDesc"]) . "</a>"; ?></td>
                                <td  align="left"><?php
                                
                                 
                                if(strlen($row["vTitle"])>50) 
								{
								 $title = substr(htmlentities($row["vTitle"]),0,50)."...";
								}
								else 
								{
									$title = $row["vTitle"];
								}
                echo "<a href='" . $detailfile . "?saleid=" . $row["nSaleId"] . "&source=sa' class='$style'>" . $title . "</a>";
                if ($row["vFeatured"] == "Y") {
                    echo "&nbsp;<img src=images/featured_star.png>";
                }//end if
                ?>
                                </td>
                                <td align="left"><?php echo "<a href='" . $detailfile . "?saleid=" . $row["nSaleId"] . "&source=sa' class='$style'>" . date('m/d/Y', strtotime($row["dPostDate"])) . "</a>"; ?></td>
                                <td align="left"><?php echo "<a href='" . $detailfile . "?saleid=" . $row["nSaleId"] . "&source=sa' class='$style'>" . $showTitleEdit . "</a>"; ?> &nbsp; <?php echo "<a href='" . $detailfile . "?saleid=" . $row["nSaleId"] . "&source=sa' class='$style clsDelete' >" . $showTitleDel . "</a>"; ?></td>
                            </tr>
                                    <?php
                                    $cnt++;
                                }//end while
