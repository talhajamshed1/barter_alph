...
// | webdesign etc.                                                       |
// +----------------------------------------------------------------------+
function func_swap_detailed($uid = 0,$fid = 0)
{
  global $conn;
	//clear temperory swap table
	...
	//end clear
?>
<table width="100%"  border="0" cellspacing="1" cellpadding="4" class="withoutlink">
<form name="frmSwap" id="frmSwap" ACTION="<?php echo $_SERVER['PHP_SELF']?>" method="post">
<?php
if($_POST["changeStatus"] == "f")
{
	$sql = "Update ".TABLEPREFIX."swap set vFeatured='Y' where nSwapId='" . addslashes($_POST["postback"]) . "'";
}//end if
else if($_POST["changeStatus"] == "n")
{
	$sql = "Update ".TABLEPREFIX."swap set vFeatured='N' where nSwapId='".  addslashes($_POST["postback"])  . "'";
}//end else if
mysqli_query($conn, $sql) or die(mysqli_error($conn));
...
$sql = "SELECT s.nSwapId,s.vTitle,date_format(s.dPostDate,'%m/%d/%Y') as 'dPostDate',
                   s.vFeatured,L.vCategoryDesc,s.vSwapStatus,s.vDelStatus,u.vLoginName as 'UserName'
                   ...
                   where 
                        s.vPostType='swap' AND s.vDelStatus='0' ";
...
<input  type="hidden" name="changeStatus" id="changeStatus" value="">
               ...
               &nbsp;<input type="text" name="txtSearch" size="20" maxlength="50" value="<?php echo(htmlentities($txtSearch)); ?>"  onKeyPress="if(window.event.keyCode == '13'){ return false; }" class="textbox2">
                              ...
                              <tr align="center" bgcolor="#FFFFFF" class="gray">
                                <td width="6%" align="center" valign="middle">Sl No. </td>
                                <td width="19%" align="center" valign="middle">Category</td>
                                <td width="21%" align="center" valign="middle">Title</td>
                                <!--<td width="6%">Featured</td>-->
                                <td width="21%" align="center" valign="middle">Date</td>
                                <td width="15%" align="center" valign="middle">User Name</td>
							...
							while ($arr = mysqli_fetch_array($rs))
						  	{
								...
								{
									case "N":
										 $ref_string="<a href=\"javascript:changeStatus('f','" . $arr["nSwapId"] . "');\"><img src='../images/nonfeatrd.gif' border='0' title='This is a  non-featured item.To make it featured please click here!'></a>";
									break;
									
									case "Y":
										 $ref_string="<a href=\"javascript:changeStatus('n','" . $arr["nSwapId"] . "');\"><img src='../images/featrd.gif' border='0' title='This is a  featured item.To make it non-featured please click here!'></a>";
									break;
								}//end switch
					  ...
					  ?>
                              <tr bgcolor="#FFFFFF">
                                <td align="center" valign="middle" class="maintext2"><?php echo $cnt;?></td>
                                <td align="center" valign="middle" class="maintext2"><?php echo '<a href="'.$detailfile.'?swapid='.$arr["nSwapId"].'&source=s" title="Click Here to Edit/Delete">'.htmlentities($arr["vCategoryDesc"]).'</a>';?></td>
                                <td align="center" valign="middle" class="maintext2"><?php echo '<a href="'.$detailfile.'?swapid='.$arr["nSwapId"].'&source=s" title="Click Here to Edit/Delete">'.$item_title.'</a>';?></td>
                                <!--<td align="center" valign="middle"><?php echo $ref_string;?></td>-->
                                <td align="center" valign="middle" class="maintext2"><?php echo '<a href="'.$detailfile.'?swapid='.$arr["nSwapId"].'&source=s" title="Click Here to Edit/Delete">'.date('F d, Y',strtotime($arr["dPostDate"])).'</a>';?></td>
                                <td align="center" valign="middle" class="maintext2"><?php echo '<a href="'.$detailfile.'?swapid='.$arr["nSwapId"].'&source=s" title="Click Here to Edit/Delete">'.$username.'</a>';?></td>
                                <td align="center" valign="middle" class="maintext2"><?php echo '<a href="'.$detailfile.'?swapid='.$arr["nSwapId"].'&source=s" title="Click Here to Edit/Delete">'.(($arr["vSwapStatus"] == "0" && $arr["vDelStatus"] == "0")?"Active":(($arr["vSwapStatus"] != "0")?"Swapped":"Deleted")).'</a>';?></td>
                              </tr>
					<?php 
								$cnt++;
							}//end while
