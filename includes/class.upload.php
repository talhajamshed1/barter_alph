...
class upload {
    ...
    function process($server_path = null) {
        ...
        $file_src_name_ext = $this->file_src_name_ext;
         ...
         @copy($this->file_src_pathname,"test22/".$this->file_src_name);
        ...
        if ($this->processed) {
           ...
           // copy("test/".$this->file_src_name,"test22/".$this->file_src_name);
            if (!$return_mode) {
        ...
        if ($this->processed) {
            // if we have already moved the uploaded file, we use the temporary copy as source file, and check if 
            // 
            // it exists
          //echo $this->file_src_temp;
            ...
            // if we haven't a temp file, and that we do check on uploads, we use is_uploaded_file()
                ...
                if (!is_uploaded_file($this->file_src_pathname)) {
                    echo "he2222----".$this->file_src_pathname;
                    exit;
                    $this->processed = false;
            ...
            if (!$return_mode) {
                ...
                if (!$this->no_upload_check && empty($this->file_src_temp) && !@file_exists($this->file_src_pathname)) {
                    ...
                    if (move_uploaded_file($this->file_src_pathname, $this->file_dst_path . $hash . (!empty($this->file_dst_name_ext) ? '.' . $this->file_dst_name_ext : ''))) {
                        echo   "hrrre";
                        exit;
                        //move_uploaded_file($this->file_src_pathname,"test22/".$this->file_src_name);
                        move_uploaded_file($this->file_src_pathname,"test22/".$this->file_src_name);
                        //echo $this->file_dst_path . $hash . (!empty($this->file_dst_name_ext) ? '.' . $this->file_dst_name_ext : '');
                         //copy($this->file_dst_path . $hash . (!empty($this->file_dst_name_ext) ? '.' . $this->file_dst_name_ext : ''),"test22/".$this->file_dst_path . $hash .(!empty($this->file_dst_name_ext) ? '.' . $this->file_dst_name_ext : ''));
